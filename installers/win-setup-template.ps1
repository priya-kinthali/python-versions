...
if ($Architecture -like "*arm64*") {
    if (Test-Path $PythonArchPath) {
        ...
        Write-Host "[ARM-PATCH] Confirmed deleted: $(if (Test-Path $PythonArchPath) { "FAILED" } else { "SUCCESS" })"
    } else {
        Write-Host "[ARM-PATCH] $PythonArchPath did not exist, nothing to delete."
    }
}
...
if ($IsFreeThreaded) {
    ...
    Write-Host "Free-threaded symlink created successfully."
} else {
    ...
    Write-Host "Minor version symlink created successfully."
}
